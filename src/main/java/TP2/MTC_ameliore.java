...
public class MTC extends AbstractPlanner {
    ...
    public Plan MonteCarlo(Problem problem) {
        ...
        // Initialisation des valeurs Q pour les actions
        Map<Action, Double> actionValues = new HashMap<>();
        for (Action a : problem.getActions()) {
            actionValues.put(a, 0.0); // Initialiser Q(a) à zéro
        }
        ...
        while (!s.satisfy(goal) && (System.currentTimeMillis() - time < timeout)) {
            ...
            s = MonteCarloRandomWalks(problem, s, goal, heuristic, actionValues);
    ...
    // Monte Carlo avec amélioration MDA et MHA
    private Node MonteCarloRandomWalks(Problem problem, Node s, Condition goal, StateHeuristic heuristic, Map<Action, Double> actionValues) {
        ...
        for (int i = 0; i < NUM_WALK; i++) {
            ...
            for (int j = 0; j < LENGTH_WALK; j++) {
                ...
                // Sélection de l'action avec Gibbs Sampling
                Action selectedAction = gibbsSample(A, actionValues, tau);
                Node newNode = appliquer(sPrime, selectedAction, problem, problem.getActions().indexOf(selectedAction), heuristic);
    ...
    // Sélectionne une action en utilisant Gibbs Sampling
    private Action gibbsSample(List<Action> actions, Map<Action, Double> actionValues, double tau) {
        // Calcul des probabilités basées sur les Q-values
        List<Double> probabilities = new ArrayList<>();
        double total = 0.0;
        
        // Calcul des probabilités pour chaque action
        for (Action a : actions) {
            double Q = actionValues.getOrDefault(a, 0.0);
            double probability = Math.exp(Q / tau);
        ...
        // Sélection de l'action en fonction des probabilités
    ...
    // Pénalisation des actions échouées dans MDA
    private void updateActionValues(Map<Action, Double> actionValues, Action action, boolean isSuccess) {
        double currentQ = actionValues.getOrDefault(action, 0.0);
        if (!isSuccess) {
            actionValues.put(action, currentQ - 1); // Pénalisation en cas d'échec
        } else {
            actionValues.put(action, currentQ + 1); // Récompense en cas de succès
        }
    }
    ...
    // Vérifie si une action est utile (MHA)
    private boolean isHelpfulAction(Action action, Node sPrime, Condition goal, StateHeuristic heuristic) {
        // L'heuristique de MHA : Si l'action aide à réduire la distance au but, elle est utile
        return heuristic.estimate(sPrime, goal) < heuristic.estimate(sPrime.getParent(), goal);
    }
